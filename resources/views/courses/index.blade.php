<x-app-layout>
    <div x-data="{ 
        editingCourse: { id: null, name: '', type: 'online', max_students: '', enrollment_deadline: '' },
        errors: {},
        isLoading: false,
        selected: [],
        ...
        editCourse(course) {
            ...
            $dispatch('open-drawer-edit-course');
        },
        ...
        async submitForm(event) {
            ...
            if (!formData.get('enrollment_deadline')) {
                this.errors.enrollment_deadline = ['Este campo é obrigatório.'];
                hasErrors = true;
            }
    ...
    class="space-y-8">
        <!-- Dashboard Heading -->
        <div class="fixed top-20 right-4 z-50 bg-black/80 text-white p-2 text-xs font-mono pointer-events-none" style="display:none">
            Selected: <span x-text="JSON.stringify(selected)"></span>
        </div>
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                <h1 class="text-3xl font-black text-slate-900 tracking-tight">Cursos</h1>
                <p class="mt-2 text-sm text-slate-500 font-medium">Gerencie o catálogo de cursos e prazos de matrícula de forma eficiente.</p>
            </div>
            <div class="flex items-center gap-3">
                <button 
                    @click="$dispatch('open-drawer-new-course')"
        ...
        <!-- Side Drawer for New Course -->
        <x-side-drawer name="new-course" title="Cadastrar Novo Curso">
            <form @submit.prevent="submitForm" action="{{ route('courses.store') }}" method="POST" class="space-y-8" novalidate>
                @csrf

                <!-- Name -->
                <div>
                    <label for="name" class="text-xs font-black text-slate-400 uppercase tracking-widest block mb-2 px-1">Nome do Curso</label>
                ...
                <!-- Type -->
                <div>
                    <label for="type" class="text-xs font-black text-slate-400 uppercase tracking-widest block mb-2 px-1">Tipo de Curso</label>
                    <div class="grid grid-cols-2 gap-3 p-1.5 bg-slate-100 rounded-2xl border border-slate-200/60">
                        <label class="cursor-pointer">
                            <input type="radio" name="type" value="online" class="hidden peer" checked>
                            <div class="flex items-center justify-center py-2.5 text-xs font-bold rounded-xl border border-transparent peer-checked:bg-white peer-checked:text-slate-900 peer-checked:shadow-sm text-slate-500 transition-all">
                                Online
                            </div>
                        </label>
                        <label class="cursor-pointer">
                            <input type="radio" name="type" value="presential" class="hidden peer">
                            <div class="flex items-center justify-center py-2.5 text-xs font-bold rounded-xl border border-transparent peer-checked:bg-white peer-checked:text-slate-900 peer-checked:shadow-sm text-slate-500 transition-all">
                                Presencial
                            </div>
                        </label>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <!-- Max Students -->
                    <div>
                        <label for="max_students" class="text-xs font-black text-slate-400 uppercase tracking-widest block mb-2 px-1">Limite de Vagas</label>
                    ...
                    <!-- Deadline -->
                    <div>
                        <label for="enrollment_deadline" class="text-xs font-black text-slate-400 uppercase tracking-widest block mb-2 px-1">Prazo de Matrícula</label>
                        <input type="date" id="enrollment_deadline" name="enrollment_deadline"
                        ...
                        <template x-if="errors.enrollment_deadline">
                            <ul class="mt-2 space-y-1 text-rose-500 text-[10px] font-bold uppercase tracking-widest">
                                <template x-for="error in errors.enrollment_deadline">
                                    <li x-text="error"></li>
                                </template>
                            </ul>
                        </template>
                ...
                <div class="pt-6">
                    ...
                    <p class="mt-4 text-[10px] text-center text-slate-400 font-bold uppercase tracking-widest">
                        Dados verificados automaticamente pelo sistema.
                    </p>
                </div>
            </form>
        </x-side-drawer>

        <!-- Side Drawer for Edit Course -->
        <x-side-drawer name="edit-course" title="Editar Curso">
            <form @submit.prevent="submitForm" :action="`/courses/${editingCourse.id}`" method="POST" class="space-y-8" novalidate>
                @csrf
                @method('PUT')
                <input type="hidden" name="id" x-model="editingCourse.id">

                <!-- Name -->
                <div>
                    <label for="edit_name" class="text-xs font-black text-slate-400 uppercase tracking-widest block mb-2 px-1">Nome do Curso</label>
                    <input type="text" id="edit_name" name="name" x-model="editingCourse.name" 
                        class="block w-full px-5 py-4 border border-slate-200 rounded-2xl text-sm bg-slate-50/50 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-slate-900/5 focus:border-slate-400 transition-all shadow-sm"
                        placeholder="Ex: Desenvolvimento Web Completo">
                ...
                <!-- Type -->
                <div>
                    <label for="edit_type" class="text-xs font-black text-slate-400 uppercase tracking-widest block mb-2 px-1">Tipo de Curso</label>
                    <div class="grid grid-cols-2 gap-3 p-1.5 bg-slate-100 rounded-2xl border border-slate-200/60">
                        <label class="cursor-pointer">
                            <input type="radio" name="type" value="online" x-model="editingCourse.type" class="hidden peer">
                            <div class="flex items-center justify-center py-2.5 text-xs font-bold rounded-xl border border-transparent peer-checked:bg-white peer-checked:text-slate-900 peer-checked:shadow-sm text-slate-500 transition-all">
                                Online
                            </div>
                        </label>
                        <label class="cursor-pointer">
                            <input type="radio" name="type" value="presencial" x-model="editingCourse.type" class="hidden peer">
                            <div class="flex items-center justify-center py-2.5 text-xs font-bold rounded-xl border border-transparent peer-checked:bg-white peer-checked:text-slate-900 peer-checked:shadow-sm text-slate-500 transition-all">
                                Presencial
                            </div>
                        </label>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <!-- Max Students -->
                    <div>
                        <label for="edit_max_students" class="text-xs font-black text-slate-400 uppercase tracking-widest block mb-2 px-1">Limite de Vagas</label>
                        <input type="number" id="edit_max_students" name="max_students" x-model="editingCourse.max_students" min="1"
                            oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                            class="block w-full px-5 py-4 border border-slate-200 rounded-2xl text-sm bg-slate-50/50 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-slate-900/5 focus:border-slate-400 transition-all shadow-sm"
                            placeholder="Opcional">
                        <x-input-error :messages="$errors->get('max_students')" class="mt-2" />
                    ...
                    <!-- Deadline -->
                    <div>
                        <label for="edit_enrollment_deadline" class="text-xs font-black text-slate-400 uppercase tracking-widest block mb-2 px-1">Prazo de Matrícula</label>
                        <input type="date" id="edit_enrollment_deadline" name="enrollment_deadline" x-model="editingCourse.enrollment_deadline"
                            class="block w-full px-5 py-4 border border-slate-200 rounded-2xl text-sm bg-slate-50/50 text-slate-900 focus:outline-none focus:ring-4 focus:ring-slate-900/5 focus:border-slate-400 transition-all shadow-sm">
        ...
        <div class="bg-white rounded-[2rem] shadow-premium border border-slate-200/50 overflow-hidden">
            ...
            <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/30 flex flex-col lg:flex-row justify-between items-center gap-6">
                <div class="flex items-center gap-4 w-full lg:w-auto">
                    <form id="search-form" action="{{ route('courses.index') }}" method="GET" class="relative group flex-1 lg:flex-none flex items-center gap-3">
                        <!-- Hidden Type Input -->
                ...
                <div class="flex items-center gap-4 w-full lg:w-auto justify-end">
                    <input type="date" 
                        ...
                        class="bg-white border border-slate-200 text-slate-900 text-xs font-bold rounded-2xl px-4 py-3 focus:ring-4 focus:ring-slate-900/5 focus:border-slate-400 shadow-sm transition-all"
                        title="Filtrar por Prazo de Matrícula">
                        
                    <div class="flex items-center bg-white border border-slate-200 rounded-2xl p-1 shadow-sm">
                        <button type="button" @click="type = 'all'; $nextTick(() => document.getElementById('search-form').submit())" 
                            :class="type === 'all' ? 'text-slate-900 bg-slate-50 shadow-inner-glow' : 'text-slate-400 hover:text-slate-600'"
                            class="px-3 py-2 text-xs font-bold rounded-xl transition-all">Todos</button>
                        <button type="button" @click="type = 'online'; $nextTick(() => document.getElementById('search-form').submit())" 
                            :class="type === 'online' ? 'text-slate-900 bg-slate-50 shadow-inner-glow' : 'text-slate-400 hover:text-slate-600'"
                            class="px-3 py-2 text-xs font-bold rounded-xl transition-all">Online</button>
                        <button type="button" @click="type = 'presential'; $nextTick(() => document.getElementById('search-form').submit())" 
                            :class="type === 'presential' ? 'text-slate-900 bg-slate-50 shadow-inner-glow' : 'text-slate-400 hover:text-slate-600'"
                            class="px-3 py-2 text-xs font-bold rounded-xl transition-all">Presencial</button>
                    </div>
                    </div>
                    
                    <div class="h-8 w-[1px] bg-slate-200 mx-2 hidden lg:block"></div>

                    <select name="limit" onchange="window.location.href='{{ route('courses.index') }}?limit=' + this.value" class="appearance-none bg-white border border-slate-200 text-slate-900 text-xs font-bold rounded-2xl pl-4 pr-10 py-3 focus:ring-4 focus:ring-slate-900/5 focus:border-slate-400 shadow-sm cursor-pointer hover:bg-slate-50 transition-all">
    ...
    <div x-show="selected.length > 0" 
        ...
        <form action="{{ route('courses.massDestroy') }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir os itens selecionados?')">
            @csrf
            @method('DELETE')
